...
export default function EventsPage() {
  ...
  useEffect(() => {
    ...
    console.log("[EventsPage] Subscribing to events for org:", currentOrg.id);
    setLoading(true);
    ...
    const unsub = onSnapshot(q, {
      next: (snap) => {
        ...
        console.log(`[EventsPage] Received ${docs.length} events`);
        setEvents(docs);
      ...
      error: (err) => {
        console.error("[EventsPage] Firestore error:", err);
        setLoading(false);
      }
    });

    return () => {
      console.log("[EventsPage] Unsubscribing from events");
      unsub();
    };
  }, [currentOrg?.id]);
