...
import { DEFENSESHIELD_CONFIG } from "@/lib/orchestrator/newModuleConfigs";
...
const cfg = DEFENSESHIELD_CONFIG;
...
export default function DefensePage() {
  ...
  async function startCam(face:"user"|"environment"=facing){
    try{
      ...
      setStreaming(true);
      let org=orgId;
      if(!org){const r=await quickSetup(mod.name);org=r.organizationId;setOrgId(org);}
      if(org&&!camId){
        const id=await createCameraDirectly({organizationId:org,name:mod.name+" Camera",brand:"WebRTC",connector:"phone_webcam",timezone:Intl.DateTimeFormat().resolvedOptions().timeZone});
        setCamId(id);setLog("✅ "+mod.name+" prêt");
  ...
  async function handleRecord(){
    ...
    await setDoc(doc(db,"organizations",orgId,"events",evId),{id:evId,organizationId:orgId,cameraId:camId,siteId:"default",detectionIds:[],primaryType:"manual_recording",category:"manual",label:"Enregistrement "+mod.name,severity:"info",durationSeconds:0,thumbnailUrl:null,videoClipUrl:null,clipStatus:"recording",acknowledged:false,createdAt:now,updatedAt:now});
  ...
  const handleDetection=useCallback(async(dets:Detection[])=>{
    ...
    if(dets[0])setLog(dets[0].label+" · "+time);
  ...
  const btnBase="rounded-xl px-4 py-2.5 text-sm font-bold";
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      <div className="border-b border-slate-800 px-5 py-4">
        <div className="flex items-center gap-3 mb-3">
          ...
          <div><h1 className="text-base font-bold text-white">{mod.name}</h1><p className="text-xs text-slate-500">{mod.goal?.slice(0,55)}...</p></div>
        </div>
        ...
        {tab==="overview"&&(
          <div className="space-y-4">
            <div className="relative aspect-video overflow-hidden rounded-xl bg-black border border-slate-800">
              ...
              {streaming&&<div className="absolute top-3 left-3 flex items-center gap-1.5 rounded-full bg-black/60 px-2.5 py-1"><span className="h-2 w-2 animate-pulse rounded-full bg-red-500"/><span className="text-xs text-white">LIVE · {mod.name}</span></div>}
              ...
              {!streaming&&<div className="absolute inset-0 flex flex-col items-center justify-center gap-3"><span className="text-5xl">{mod.icon}</span><button onClick={()=>startCam()} className={btnBase+" text-white"} style={{background:mod.color}}>▶ Démarrer</button></div>}
            </div>
            <div className="flex flex-wrap gap-2">
              {!streaming?<button onClick={()=>startCam()} className={btnBase+" text-white"} style={{background:mod.color}}>▶ Démarrer</button>
              :<button onClick={stopCam} className={btnBase+" border border-red-700 bg-red-900/10 text-red-400"}>⏹ Arrêter</button>}
              ...
              <button onClick={handleRecord} disabled={!streaming||recording||uploading} className={"flex items-center gap-2 "+btnBase+" disabled:opacity-40 "+(recording?"border border-red-600 bg-red-900/20 text-red-400":"bg-red-600 text-white hover:bg-red-700")}>
                {recording?<><span className="h-2 w-2 animate-pulse rounded-full bg-red-400"/>⏹ REC...</>:uploading?"⏳...":"🔴 Enregistrer"}
              </button>
